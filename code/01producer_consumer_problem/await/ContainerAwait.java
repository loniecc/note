...
/**
 * 1. 仓库满了就不能生产 2. 仓库空了就不能消费
 */
public class ContainerAwait {
    ...
    public void get(String name) throws Exception {
        lock.lock();
        if (curCap <= 0) {
            System.out.println(name + " is  empty , take a rest and try again");
            notEmpty.await();
        } else {
            curCap--;
            System.out.println(String.format("%s consume , current size: %d", name, curCap));
            notFull.signal();
        }
        lock.unlock();
    }
